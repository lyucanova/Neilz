...
public final class Settings {
    ...
    public static Settings load(FileConfiguration config) {
        ...
        int radiationDurationSeconds = explosiveNuclear.getInt("radiation-duration-seconds", -1);
        NuclearBombSettings nuclearBombSettings = new NuclearBombSettings(Math.max(20, explosiveNuclear.getInt("charge-ticks", 120)), Math.max(8.0, explosiveNuclear.getDouble("blast-radius", 120.0)), Math.max(4.0f, (float)explosiveNuclear.getDouble("explosion-power", 36.0)), Math.max(4, explosiveNuclear.getInt("crater-radius", 30)), Math.max(2, explosiveNuclear.getInt("crater-depth", 14)), Math.max(0, explosiveNuclear.getInt("crater-height", 18)), Math.max(8.0, explosiveNuclear.getDouble("mushroom-height", 120.0)), Math.max(4.0, explosiveNuclear.getDouble("mushroom-radius", 52.0)), radiationStartRadius, radiationMaxRadius, radiationDurationSeconds < 0 ? -1 : Math.max(60, radiationDurationSeconds), Math.max(1, explosiveNuclear.getInt("radiation-expansion-seconds", 2400)), Math.max(0.1, explosiveNuclear.getDouble("exposure-per-second", 1.0)), Math.max(0.0, explosiveNuclear.getDouble("safe-decay-per-second", 0.35)), Math.max(1.0, explosiveNuclear.getDouble("damage-threshold", 12.0)), Math.max(0.1, explosiveNuclear.getDouble("damage-per-second", 1.5)), Math.max(8.0, explosiveNuclear.getDouble("core-shockwave-radius", 120.0)));
